var x=document.getElementsByClassName("datumobjaveKom");
var ispisi=document.getElementsByClassName("datumiKomentari");
var z=document.getElementsByClassName("novostiDiv");

function racunajKomentare(){
	var y=new Date();
	var razlika;
...
var brn;
	...
	if(sekunde<60)
	{
		ispisi[i].innerHTML="Komentar objavljen prije par sekundi";
	...
	else if(sekunde>=60 && sekunde<3600)
	{
		...
		{ispisi[i].innerHTML="Komentar objavljen prije "+brn+" minutu";
		ispisi[i].style.display="block";}
		else if((n>2&&n<=5)||(n>22&&n<=25)||(n>32&&n<=35)||(n>42&&n<=45)||(n>52&&n<=55))
		{ispisi[i].innerHTML="Komentar objavljen prije "+brn+" minute";
		ispisi[i].style.display="block";}
		else if((n>5&&n<=21)||(n>25&&n<=31)||(n>35&&n<=41)||(n>45&&n<=51)||(n>55&&n<=60))
		{ispisi[i].innerHTML="Komentar objavljen prije "+brn+" minuta";
		ispisi[i].style.display="block";}
	...
	else if(sekunde>=3600 && sekunde<86400)
	{
		...
		if((n<=2)||(n>21&&n<=22))
		{
			ispisi[i].innerHTML="Komentar objavljen prije "+brn+" sat";
		    ispisi[i].style.display="block";
		}
		else if((n>2 && n<=4)||(n>22 && n<=24))
		{
			ispisi[i].innerHTML="Komentar objavljen prije "+brn+" sata";
		    ispisi[i].style.display="block";
		}
		else if(n>5 && n<=24)
		{
			ispisi[i].innerHTML="Komentar objavljen prije "+brn+" sati";
		    ispisi[i].style.display="block";
		}
	...
	else if(sekunde>=86400 && sekunde<604800)
	{
		...
		brn=Math.round(n);
		if(brn==1)
		{ispisi[i].innerHTML="Komentar objavljen prije 1 dan";
		ispisi[i].style.display="block";}
		else{
			 
			ispisi[i].innerHTML="Komentar objavljen prije "+brn+" dana";
			ispisi[i].style.display="block";
		}
	...
	else if(sekunde>=604800 && sekunde<2592000)
	{
		...
		if(brn==1)
		{
			ispisi[i].innerHTML="Komentar objavljen prije 1 sedmicu";
			ispisi[i].style.display="block";
		}
		else
		{	
			ispisi[i].innerHTML="Komentar objavljen "+brn+" sedmice.";
			ispisi[i].style.display="block";
		}
